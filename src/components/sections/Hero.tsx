  ...
  return (
    ...
    >
      ...
      {/* Overlay liviano: oscurece solo lo necesario para leer el texto de
          abajo, dejando la foto visible arriba en vez de taparla entera. */}
      <div className="absolute inset-0 bg-gradient-to-t from-espresso-deep via-espresso-deep/40 to-transparent" />

      <div className="relative mx-auto w-full max-w-6xl px-6 pb-14 sm:px-8 sm:pb-20 md:pb-24">
        ...
        <Reveal delay={160}>
          <p className="mt-4 max-w-xs text-base leading-relaxed text-cream/80 sm:max-w-sm sm:text-lg">
            {heroTagline}
          </p>
        </Reveal>

        <Reveal delay={200} className="mt-4 flex items-center gap-2 text-sm text-cream/75">
          <span className="h-1.5 w-1.5 rounded-full bg-gold" aria-hidden="true" />
          {business.hoursToday}
        </Reveal>

        <Reveal delay={280} className="mt-8 flex flex-wrap items-center gap-x-7 gap-y-3">
          <Button href="/menu" variant="primary" className="px-8 py-3.5 text-sm">
            Ver el Menú
          </Button>
          <a
            href="#ubicacion"
            className="text-sm font-medium text-cream/70 underline-offset-4 transition-colors hover:text-cream hover:underline"
          >
            Cómo llegar
          </a>
        </Reveal>
